...
Before designing, read the error register: the memory index
`.claude/memory/MEMORY.md` (**the index only** — never the individual memory
files; each entry's description states when it fires) and, if the source
reworks file has one, its "Tracked observations" section. These record design
mistakes already made; do not design defensively against ones that do not
apply to your finding.
...
1. **Each finding is fix-sized:** one bounded diff plus its regression test,
   ...
   it is too big — split it. For rendering steps: a step that introduces two
   or more new render passes/pipeline families, or a new pass plus its consumer
   calibration, should split at pass boundaries (each pass independently
   provable via readback) — a two-pass step measured 180k output vs the 27k
   campaign median (2026-07-16), and splitting keeps worker contexts in the
   ~100k band. A step whose Path touches no source code and names no test
   (diagrams, BASELINE tables, queue notes) gets "(docs-only)" appended to
   its title — the orchestrator routes those to a cheaper model.
2. **Each finding leaves the workspace green:** compiling, zero new warnings,
   ...
   state. Order steps so this holds. A step whose diff is content-only (adding
   or changing files under `content/` or test fixture directories like `tests/data/`)
   must still name the full gate in its Path, since content is consumed cross-crate
   by lints and render tests.
...
4. **Tests are behavioral:** each step's Path names a test that exercises the
   ...
   constants asserted, no logic re-implemented inline in the test — and names
   the assertion itself: the predicate and the expected value, so a test that
   constructs the right scenario but checks something weaker is visibly
   half-landed rather than green.
5. **The plan absorbs the thinking:** steps are executed by a lower-tier
   ...
   here, in your design pass. Never leave the worker a question it would have
   to explore to answer.
6. **Discovery includes measurement — do not guess a value you could fetch.**
   You have full tool access; use it before you write, not after. Every number
   and every "is true that" the plan leans on must trace to one of three
   sources: measured by you in this pass, read from an artifact already on disk
   (a stats JSON, a log, a shipped mesh), or explicitly labelled a hypothesis
   whose test is **step 1**. "It stands to reason" is not a fourth source, and a
   plan carrying one is not ready to be written. Branching a Path across
   plausible outcomes is a last resort, honest only when the value cannot exist
   until the step runs — it is not a substitute for going and looking. Two traps
   that survive an otherwise careful measurement: the instrument you test a
   premise with must not be the mechanism that premise justifies, and a
   criterion with a free parameter (direction/sample/ray count, resolution,
   tolerance) is unmeasured until you show the answer stable as that parameter
   is refined toward its limit. Never write a Path that instructs a worker to
   move a threshold so a metric passes.
